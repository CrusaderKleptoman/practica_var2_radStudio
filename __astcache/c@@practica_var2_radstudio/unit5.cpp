...
//Переменная с наименьшей разницей площадей
double minRaz;
//Массив, хранящий номера точек выполняющих условия задания
unsigned short pointNumber[3] = {0, 1, 2};
//Переменная записывающая возможность построить треугольник
bool trianglExists = false;
//Вектор хранящий точки на плоскости
vector<TPoint> points;
//Метод расчитывающий растояние между точками
...
//Метод рисующий круги на плоскости
...
//Метод расчитывающий точки выполняющие условия задания
void Logic(TPaintBox *PaintBox1, vector<TPoint> points) {
	 if(points.size()>=3){
		PaintBox1->Canvas->Pen->Color = clRed;
		minRaz = abs(S_circle({1, 1}, {PaintBox1->Width, 1}, {1, PaintBox1->Height}) - S_triangl({1, 1}, {PaintBox1->Width, 1}, {1, PaintBox1->Height}));
	 ...
	 {
		ShowMessage("Введено недостаточно точек");
		return;
	 }
	 if(trianglExists)
	 {
	 ...
	 {
		ShowMessage("По данным точкам невозможно построить треугольник и круг");
	 }
}
//Метод отрисовывающий поверхность и точки на ней
...
void __fastcall TForm5::ButtonEnterCoordinateClick(TObject *Sender)
{
	...
	for(int i; i < points.size(); i++)
	{
		...
		{
			ShowMessage("По данным координатам уже существует точка.");
			return;
		}
	}
	...
	{
		ShowMessage("Координаты введённой точки выходят за пределы графической поверхности.");
	}
	StaticTextPoint1X->Caption = "";
	 StaticTextPoint1Y->Caption = "";
	StaticTextPoint2X->Caption = "";
...
//---------------------------------------------------------------------------
//Метод, активирующий решение задачи
...
//Метод, выводящий текст задания
void __fastcall TForm5::ButtonTaskClick(TObject *Sender)
{
	ShowMessage("Из заданного на плоскости множества точек выбрать три различные точки так, чтобы разность между площадью круга, ограниченного окружностью, проходящей через эти три точки, и площадью треугольника с вершинами в этих точках.");
}
//---------------------------------------------------------------------------
=======
//---------------------------------------------------------------------------
...
void __fastcall TForm5::ButtonEnterCoordinateClick(TObject *Sender)
{
	...
	{
		ShowMessage("Координаты введёной точки выходят за пределы графической поверхности.");
	}
	StaticTextPoint1X->Caption = "";
	 StaticTextPoint1Y->Caption = "";
...
>>>>>>> a103720fa14afd2e4187ee9952ed307b007e4ce8
